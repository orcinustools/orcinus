...
| `configs` | `ConfigMap` + mount at the target | relative `file:` resolves against the compose file's dir |
| `secrets` | `Secret` | `file:` creates one from that file, `external: true` uses one already in the cluster. Consume it as a mounted file (list it under the service), as env vars (`x-orcinus-env-from-secret`), or both — see [COMPOSE.md](./COMPOSE.md#importing-a-file-into-a-secret) |
...
| `x-orcinus-env-from-secret` | secret name or list | Load a Secret's keys into the container env (`envFrom`). The Secret is not generated by this key — it comes from `orcinus secret create`, or from a `secrets: {name: {file: …}}` entry in the same compose file |
